...
public interface ICountryLookupService
{
    ...
    /// <summary>
    /// Attempts to convert a Czech country name (as used on CSFD) to its ISO alpha-2 code.
    /// </summary>
    /// <param name="czechName">The country name in Czech.</param>
    /// <returns>The ISO alpha-2 code if found; otherwise, null.</returns>
    string? GetIsoCodeFromCzechName(string czechName);
}
...
public class CountryLookupService : ICountryLookupService
{
    ...
    public string? GetIsoCodeFromCzechName(string czechName)
    {
        if (string.IsNullOrWhiteSpace(czechName)) return null;

        var trimmed = czechName.AsSpan().Trim();
        string normalized;
        if (trimmed.Length == 0)
        {
        ...
        {
            Span<char> lowerBuffer = stackalloc char[trimmed.Length];
            trimmed.ToLowerInvariant(lowerBuffer);
            normalized = new string(lowerBuffer);
        }

        if (CsfdCzechNameToIsoAlpha2.TryGetValue(normalized, out var isoCode))
        {
            return isoCode; 
        }

        var asciiNormalized = StripDiacritics(normalized.AsSpan());
        return CsfdCzechNameToIsoAlpha2Normalized.TryGetValue(asciiNormalized, out isoCode) ? isoCode : null;
    }

    private static Dictionary<string, string> BuildNormalizedCzechNameMap()
    {
        var map = new Dictionary<string, string>(StringComparer.Ordinal);
        foreach (var kvp in CsfdCzechNameToIsoAlpha2)
        {
            var normalizedKey = StripDiacritics(kvp.Key.AsSpan());
            if (!map.ContainsKey(normalizedKey))
            {
                map[normalizedKey] = kvp.Value;
            }
        }
    ...
    private static string StripDiacritics(ReadOnlySpan<char> value)
    {
        var normalized = value.ToString().Normalize(NormalizationForm.FormD);
